...
export default function JobDescription() {
  ...
  useEffect(() => {
    api.listDocuments('job_description').then(r => setDocuments(r.documents || [])).catch(() => {});
  }, []);
  ...
  const handleFile = async (file: File) => {
    ...
    setError('');
    try {
      await api.uploadDocument(file, 'job_description');
      setStatus('indexed');
      api.listDocuments('job_description').then(r => setDocuments(r.documents || [])).catch(() => {});
    } catch (e: any) {
      setStatus('error');
  ...
  const deleteDoc = async (id: string) => { await api.deleteDocument(id); setDocuments(d => d.filter(doc => doc.id !== id)); };
  ...
  return (
    <div>
      ...
      <div style={{ ...styles.dropzone, borderColor: dragOver ? 'var(--accent)' : 'var(--border-default)', background: dragOver ? 'var(--accent-soft)' : 'var(--bg-elevated)' }}
        ...
        {status === 'error' && <p style={{ ...styles.dropText, color: 'var(--error)' }}>{error || 'Failed'}</p>}
      </div>

      {documents.length > 0 && (
        <div>
          <h3 style={styles.sectionTitle}>Uploaded Job Descriptions</h3>
          {documents.map(doc => (
